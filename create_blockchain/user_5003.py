...
def mine_block():
    ...
    blockchain.add_transaction(sender = node_address, receiver = 'user_5003', amount = 1)
...
# Replacing the chain by the longest chain if required
@app.route('/replace_chain', methods = ['GET'])
def replace_chain():
    is_chain_replaced = blockchain.replace_chain()
    if is_chain_replaced:
        response = {'message' : 'The node had different chain, so the chain is replaced by largest one.',
